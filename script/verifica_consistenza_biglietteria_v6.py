...
# 7) Fiscal checks - LOG IVA/ISI & ImponibileIntrattenimenti
    ...
    for rr in rpm_recs:
        ...
        elif kind == "BigliettiAbbonamento":
            ok = (
                (rr.get("Organizzatore_CF") or "").strip(),
                (rr.get("CodiceLocale") or "").strip(),
                (rr.get("DataEvento") or "").strip(),
    ...
    for ok, od_rr in rpm_order_info.items():
        ...
        inc = inc_map.get(ek)
        if inc is None:
            # senza incidenza non posso fare lo split; assumo 0 (solo spettacolo) per evitare crash
            inc = 0

        code = int(event_code_map.get(ek, 0) or 0)
        iva_rate, isi_rate = rates_for_code(code)

        # omaggi net RPM nell'ordine (solo O* a 0,00)
        om_rpm_net = 0
        ...
        # prezzo massimo di riferimento (corrispettivo senza prevendita) o rateo abbonamento
        max_price = 0
        for tr in rpm_tickets_by_order.get(ok, []):
            tipo = (tr.get("TipoTitolo") or "").strip()
        ...
        unit_calc = calc_intr_sp_from_gross(max_price, int(inc), float(iva_rate), float(isi_rate))
    ...
    if ev_tables:
        ...
        <details open>
          ...
          <div class=\"hint\">
            ...
            <b>Nota:</b> nei totali dell'evento la riga “*” è inclusa solo per gli importi fiscali (non nei conteggi titoli).
          </div>
          {''.join(blocks)}
        </details>
        """
