...
def build_html_report(summary: Dict[str, Any], issues: List[Issue], metrics: Dict[str, Any], details: Dict[str, Any]) -> str:
    ...
    if ev_tables:
        ...
        ev_tables_html = f"""
        <details open>
          <summary><b>Riepilogo fiscale per evento e tipologia titolo (RPM, netto annulli + *eccedenza omaggi*)</b> (eventi: {len(ev_tables_sorted)})</summary>
          <div class="hint">
            Valori calcolati dallo script sugli aggregati RPM (Corrispettivo+Prevendita), usando Incidenza da RPM e aliquote da TAB.1.
