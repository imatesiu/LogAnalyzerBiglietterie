...
def parse_aliquote_tab1_pdf(path: str) -> Dict[int, Dict[str, Any]]:
    """Parsa TAB.1 da PDF (testo) quando non è disponibile il CSV.

    La tabella contiene tipicamente colonne: Codice, Descrizione, IVA, ISI.
    Gestisce casi come:
      - IVA: "10%" / "22%" / "10 o 22%(1)" / "Esente"
      - ISI: "16%" / "60%" / "-"

    Ritorna:
      code -> {"descrizione": str, "iva_candidates": [float...], "isi": float}
    dove le aliquote sono in forma di rapporto (es. 0.22).
    """
    try:
        import PyPDF2
    except Exception as e:
        raise RuntimeError("PyPDF2 non disponibile per leggere PDF") from e
    ...
    for raw_line in text.split("\n"):
        ...
        # match codice iniziale (es: 05 oppure 41(3))
        ...
        # Cerco IVA e ISI in coda (righe complete). Esempi: "10% -" / "10 o 22%(1) -" / "22% 16%" / "Esente(2) 10%".
        m2 = re.search(
            r"\s(Esente\(?\d*\)?|\d+\s*o\s*\d+%\(?\d*\)?|\d+%)\s+(-|\d+%|Esente\(?\d*\)?)\s*$",
            rest,
            flags=re.IGNORECASE,
        )
        if not m2:
            # talvolta la riga è spezzata: la gestiamo in modo conservativo (skip)
            continue

        iva_raw = m2.group(1).strip()
        isi_raw = m2.group(2).strip()
        descr = rest[: m2.start()].strip()

        # IVA candidates
        iva_candidates = []
        if iva_raw == "-" or "esente" in iva_raw.lower():
            iva_candidates = [0.0]
        else:
            if "o" in iva_raw:
                nums = re.findall(r"(\d+(?:\.\d+)?)\s*%", iva_raw)
                iva_candidates = [float(n) / 100.0 for n in nums]
            else:
                p = _extract_percent_from_text(iva_raw)
                iva_candidates = [p] if p is not None else [0.0]
        if not iva_candidates:
            iva_candidates = [0.0]

        # ISI
        isi = 0.0
        if isi_raw == "-" or "esente" in isi_raw.lower():
            isi = 0.0
        else:
            p = _extract_percent_from_text(isi_raw)
            isi = float(p) if p is not None else 0.0
...
def _find_tab1_pdf(directory: str) -> str:
    """Prova a trovare automaticamente un PDF TAB.1 nella cartella."""
    try:
        for fn in os.listdir(directory):
            low = fn.lower()
            if low.endswith('.pdf') and ('tab. 1' in low or 'tab.1' in low or 'tabella 1' in low):
...
def parse_log(path: str) -> List[Dict[str, Any]]:
    ...
    for tr in root.findall(".//Transazione"):
        ...
        rec = {
            ...
            # Campi opzionali (presenti in alcune implementazioni): per supportare calcoli fiscali anche in assenza di RPM
            "Incidenza": to_int(tr.get("Incidenza") or ta.get("Incidenza") or _find_text(ta, "Incidenza"), 0),
            "Capienza": to_int(tr.get("Capienza") or ta.get("Capienza") or _find_text(ta, "Capienza"), 0),
            "DenominazioneLocale": _find_text(ta, "DenominazioneLocale"),
        }
        out.append(rec)
...
def parse_rpm(path: str) -> List[Dict[str, Any]]:
    ...
    for org in root.findall(".//Organizzatore"):
        ...
        for ev in org.findall("./Evento"):
            ...
            # MultiGenere (serve per ricostruire in modo deterministico il TipoGenere quando non abbiamo LOG)
            multi_genere: List[Dict[str, Any]] = []
            for mg in ev.findall("./MultiGenere"):
                tg = to_int(_find_text(mg, "TipoGenere"), 0)
                incg = to_int(_find_text(mg, "IncidenzaGenere"), 0)
                titolo_opera = _find_text(mg, "TitoliOpere/Titolo")
                if tg:
                    multi_genere.append({
                        "TipoGenere": tg,
                        "IncidenzaGenere": incg,
                        "Titolo": titolo_opera,
                    })
...
# -------------------------
# LOG selection (per giorno: prendi solo il contatore più alto)
# -------------------------

_LOG_NAME_RE = re.compile(r"^LOG_(\d{4})_(\d{2})_(\d{2})_(\d+)\.(?:xsi|xml)$", re.IGNORECASE)


def parse_log_filename(path: str) -> Optional[Tuple[int, int, int, int]]:
    """Estrae (anno, mese, giorno, contatore) dal basename.
    Esempio: LOG_2026_02_26_001.xsi -> (2026, 2, 26, 1)
    Ritorna None se il nome non rispetta il pattern.
    """
    fn = os.path.basename(path)
    m = _LOG_NAME_RE.match(fn)
    if not m:
        return None
    return int(m.group(1)), int(m.group(2)), int(m.group(3)), int(m.group(4))


def select_latest_log_per_day(log_paths: List[str]) -> Tuple[List[str], List[str], Dict[str, Any]]:
    """Per ogni giorno (YYYY-MM-DD) seleziona il LOG con contatore più alto.
    Ritorna (selected, ignored, info).
    - selected: lista path (ordinata per data); include anche eventuali file con nome non conforme (in coda), per non perderli.
    - ignored: lista path scartati perché versione inferiore dello stesso giorno.
    - info: dettagli (mapping per giorno, non conformi).
    """
    best: Dict[Tuple[int, int, int], Tuple[int, str]] = {}
    groups: Dict[Tuple[int, int, int], List[Tuple[int, str]]] = defaultdict(list)
    non_conformi: List[str] = []

    for p in log_paths:
        key = parse_log_filename(p)
        if key is None:
            non_conformi.append(p)
            continue
        y, mo, d, c = key
        k = (y, mo, d)
        groups[k].append((c, p))
        cur = best.get(k)
        if cur is None or c > cur[0] or (c == cur[0] and os.path.basename(p) > os.path.basename(cur[1])):
            best[k] = (c, p)
    ...
    selected = sorted(selected, key=lambda p: (parse_log_filename(p) or (9999, 99, 99, 999999)))
    selected.extend(sorted(non_conformi))

    info = {
        "days_total": len(groups),
        "days": sorted([f"{y:04d}-{mo:02d}-{d:02d}" for (y, mo, d) in groups.keys()]),
        "selected_map": {f"{y:04d}-{mo:02d}-{d:02d}": os.path.basename(best[(y, mo, d)][1]) for (y, mo, d) in groups.keys()},
        "ignored": sorted([os.path.basename(p) for p in ignored]),
        "non_conformi": sorted([os.path.basename(p) for p in non_conformi]),
    }
    return selected, ignored, info


def write_merged_log_xml(selected_log_paths: List[str], out_path: str) -> None:
    """Scrive un XML unificato concatenando le transazioni dei LOG selezionati.
    Uso tipico: debug/audit. Per l'analisi lo script unisce sempre in memoria.
    Strategia conservativa:
    - usa la root del primo file
    - appende i children-root degli altri file
    """
    if not selected_log_paths:
        raise ValueError("Nessun LOG selezionato")
    base_tree = ET.parse(selected_log_paths[0])
    base_root = base_tree.getroot()
    for p in selected_log_paths[1:]:
        t = ET.parse(p)
        r = t.getroot()
        for child in list(r):
...
def run_checks(
    ...
    details: Dict[str, Any] = {
        ...
        "fiscale_evento_titoli_grand_totals": {},  # totale generale (include abbonamenti + eccedenza)
    }
...
# 7) Fiscal checks - LOG IVA/ISI & ImponibileIntrattenimenti
    ...
    if aliquote is None:
        ...
        # Build event incidenza map: prefer RPM, fallback su campo (opzionale) nei LOG
        inc_map: Dict[Tuple[str, str, str, str], int] = {}
        if rpm_event_info:
            for k, ev in rpm_event_info.items():
                inc = ev.get("Incidenza")
        ...
        metrics["FIS_LOG_skipped_tassazione_S"] = skipped_tassazione
    ...
    # Helper: riconosce una transazione LOG di annullamento
    def is_log_annullamento(r: Dict[str, Any]) -> bool:
        return bool((r.get('OriginaleAnnullato') or '').strip() or (r.get('CartaOriginaleAnnullato') or '').strip() or ((r.get('Annullamento') or '').upper() == 'S'))
    ...
    for ek, rows in log_events.items():
        for r in rows:
            sign = -1 if is_log_annullamento(r) else 1
    ...
    for ok in sorted(all_order_keys):
        ...
        # Capienza/Incidenza: se RPM presente usa RPM, altrimenti prova dai LOG (campi opzionali)
        if rpm_recs:
            ...
            order_rr = None
            event_rr = None
            cap = max([int(r.get('Capienza') or 0) for r in rows] + [0])
            capienza = cap if cap > 0 else None
            inc0 = max([int(r.get('Incidenza') or 0) for r in rows] + [0])
            inc = inc0 if inc0 > 0 else None
            tass = (rows[0].get('TipoTassazione') or '').upper() if rows else ''
            if inc is None and tass == 'S':
                inc = 0
            rpm_iva_ecc = None
            rpm_imp_intr_event = None

        om_log = count_omaggi_net(rows) if rows else 0
        om_rpm = rpm_omaggi_net.get(ok) if ok in rpm_omaggi_net else None

        if capienza is None:
            issues.append(Issue(severity='WARN', check='FIS_LOG_OMAGGI', message='LOG-only: capienza non disponibile (impossibile calcolare eccedenza omaggi)', context={'evento': f'{data} {ora} {loc} Ord={ordine}'}))
            continue
        if inc is None:
            issues.append(Issue(severity='WARN', check='FIS_LOG_OMAGGI', message='LOG-only: incidenza non disponibile (impossibile calcolare split Intr/Sp per eccedenza omaggi)', context={'evento': f'{data} {ora} {loc} Ord={ordine}'}))
            continue
        ...
        if max_price <= 0 and ecc > 0:
            issues.append(Issue(severity='WARN', check='FIS_LOG_OMAGGI', message='Eccedenza omaggi presente ma prezzo massimo di riferimento non determinabile dal LOG (no prevendita): impossibile calcolare IVA/ISI eccedenza', context={'evento': f'{data} {ora} {loc} Ord={ordine}', 'eccedenza': int(ecc)}))
            continue
        ...
        if max_gen in resolved_rates:
            iva_rate, isi_rate = resolved_rates.get(max_gen, (0.0, 0.0))
        elif aliquote is not None and int(max_gen) in aliquote:
            cand = aliquote[int(max_gen)].get('iva_candidates', [0.0]) or [0.0]
            iva_rate = float(cand[0])
            isi_rate = float(aliquote[int(max_gen)].get('isi', 0.0) or 0.0)
        else:
            iva_rate, isi_rate = (0.0, 0.0)

        # calcolo unitario (prezzo titolo senza prevendita) con split Intrattenimento/Spettacolo
        unit_calc = calc_intr_sp_from_gross(max_price, int(inc), iva_rate, isi_rate)
        quota_intr_unit = unit_calc["quota_intr"]
        quota_sp_unit = unit_calc["quota_sp"]
        iva_intr_unit = unit_calc["iva_intr"]
        iva_sp_unit = unit_calc["iva_sp"]
        ...
        details["fiscale_omaggi"].append({
            ...
            "QuotaSp_unit": quota_sp_unit,
            "ImponibileIntr_unit": imponibile_intr_unit,
            "ImponibileSp_unit": imponibile_sp_unit,
            "IVAIntr_unit": iva_intr_unit,
            "IVASp_unit": iva_sp_unit,
            "ISI_unit": isi_unit,
            "Soglia_applicata": soglia_flag,
            "Soglia_applicata_intr": soglia_intr,
            "Soglia_applicata_sp": soglia_sp,
        ...
        if rpm_iva_ecc is not None and int(rpm_iva_ecc) != int(expected_iva_ecc):
            ...
            issues.append(Issue(
                severity=sev,
                check="FIS_RPM_OMAGGI",
                message="RPM: IVAEccedenteOmaggi non coerente con calcolo eccedenza omaggi (quota Intrattenimento/Spettacolo, no prevendita)",
                context={
                    "evento": f"{data} {ora} {loc} Ord={ordine}",
                    "capienza": capienza,
                    "omaggi_net": om_log,
                    ...
                    "incidenza": int(inc),
                    "tipo_genere": max_gen,
                    "quota_intr_unit": quota_intr_unit,
                    "quota_sp_unit": quota_sp_unit,
                    "soglia_intr": soglia_intr,
                    "soglia_sp": soglia_sp,
                    "iva_unit_intr": iva_intr_unit,
                    "iva_unit_sp": iva_sp_unit,
                    "iva_unit_ecc": iva_unit_ecc,
                    "imponibile_unit_intr": imponibile_intr_unit,
                    "imponibile_unit_sp": imponibile_sp_unit,
    ...
    # I totali dell'evento includono le componenti fiscali della riga "*" (ma NON sommano il conteggio titoli della riga "*" nei totali dei titoli).

    def rates_for_code(tipo_genere: int) -> Tuple[float, float]:
        """Restituisce (iva_rate, isi_rate) per un TipoGenere.
        Se non risolto via LOG, usa la TAB.1 (primo candidato IVA) come fallback.
        """
        if tipo_genere in resolved_rates:
            return resolved_rates[tipo_genere]
        if aliquote is not None and tipo_genere in aliquote:
            cand = aliquote[tipo_genere].get("iva_candidates", [0.0]) or [0.0]
            isi = float(aliquote[tipo_genere].get("isi", 0.0) or 0.0)
    ...
    for ek, ev_rr in rpm_event_info.items():
        ...
        best_code = 0
        best_key = (-1, -1, -1)  # (has_isi, incidenza_genere, codice)
        for mg in mg_list:
            code = int(mg.get("TipoGenere") or 0)
            incg = int(mg.get("IncidenzaGenere") or 0)
            isi = 0.0
            if aliquote is not None and code in aliquote:
                isi = float(aliquote[code].get("isi", 0.0) or 0.0)
            key = (1 if isi > 0.0 else 0, incg, code)
            if key > best_key:
                best_key = key
                best_code = code
    ...
    rpm_abbo_by_event: Dict[Tuple[str, str, str, str], List[Dict[str, Any]]] = defaultdict(list)
    for rr in rpm_recs:
        kind = rr.get("kind")
        if kind in ("TitoliAccesso", "TitoliAnnullati", "TitoliAccessoIVAPreassolta", "TitoliIVAPreassoltaAnnullati"):
            ok = (
                (rr.get("Organizzatore_CF") or "").strip(),
                (rr.get("CodiceLocale") or "").strip(),
                (rr.get("DataEvento") or "").strip(),
            ...
            rpm_tickets_by_order[ok].append(rr)
            ...
            rpm_abbo_by_order[ok].append(rr)
    ...
    # Costruzione tabelle evento (per tutti gli eventi presenti in RPM)
    fisc_ev_tables: List[Dict[str, Any]] = []
    for ek in sorted(rpm_event_info.keys(), key=lambda x: (x[2], x[3], x[1], x[0])):
        ev_rr = rpm_event_info.get(ek)
        ...
        totals_counts = {"TitoliNetti": 0, "TitoliAnnullati": 0}
        totals_fisc = {"ImponibileImposta": 0, "ImpostaIntrattenimento": 0, "ImponibileIVA": 0, "IVALorda": 0}

        for tipo in sorted(event_type_acc.get(ek, {}).keys()):
            ...
            rows.append(row)
            totals_counts["TitoliNetti"] += int(row["TitoliNetti"])
            totals_counts["TitoliAnnullati"] += int(row["TitoliAnnullati"])
            for kf in totals_fisc.keys():
                totals_fisc[kf] += int(row.get(kf, 0))

        # Riga "*" eccedenza omaggi (se presente)
        ecc = int(ecc_by_event.get(ek, {}).get("EccedenzaTitoli", 0) or 0)
        if ecc > 0:
            ecc_row = {
                "TipoTitolo": "* Eccedenza omaggi",
                "TitoliNetti": ecc,
                "TitoliAnnullati": 0,
                "ImponibileImposta": int(ecc_by_event[ek].get("ImponibileImposta", 0) or 0),
                "ImpostaIntrattenimento": int(ecc_by_event[ek].get("ImpostaIntrattenimento", 0) or 0),
                "ImponibileIVA": int(ecc_by_event[ek].get("ImponibileIVA", 0) or 0),
                "IVALorda": int(ecc_by_event[ek].get("IVALorda", 0) or 0),
                "RowType": "eccedenza",
            }
            rows.append(ecc_row)
            # NB: nei totali fiscali includiamo la riga "*", ma NON sommiamo i titoli.
            for kf in totals_fisc.keys():
                totals_fisc[kf] += int(ecc_row.get(kf, 0))

        totals = {
            "TitoliNetti": totals_counts["TitoliNetti"],
            "TitoliAnnullati": totals_counts["TitoliAnnullati"],
            **totals_fisc,
        }
        # -------------------------
        # Biglietti Abbonamento / AbbonamentiFissi (figurativi)
        # - visualizzati dopo il Totale evento
        # - contribuiscono ai totali generali
        # -------------------------
        abbo_rows: List[Dict[str, Any]] = []
        abbo_totals = {"TitoliNetti": 0, "TitoliAnnullati": 0, "ImponibileImposta": 0, "ImpostaIntrattenimento": 0, "ImponibileIVA": 0, "IVALorda": 0}

        abbo_agg: Dict[str, Dict[str, int]] = defaultdict(lambda: {"q": 0, "imp_fig": 0, "iva_fig": 0})
        for rr in rpm_abbo_by_event.get(ek, []):
            tt = (rr.get("TipoTitolo") or "").strip() or "(n/d)"
            q = int(rr.get("Quantita") or 0)
            imp_fig = int(rr.get("ImportoFigurativo") or 0)
            iva_fig = int(rr.get("IVAFigurativa") or 0)
            if q <= 0 or imp_fig <= 0:
                continue
            abbo_agg[tt]["q"] += q
            abbo_agg[tt]["imp_fig"] += imp_fig
            abbo_agg[tt]["iva_fig"] += iva_fig

        # Aliquote per evento
        code = int(event_code_map.get(ek, 0) or 0)
        iva_rate, isi_rate = rates_for_code(code)
        inc = meta.get("Incidenza")
        inc = int(inc) if inc is not None and str(inc) != "" else 0
        tipo_tass = (meta.get("TipoTassazione") or "").strip()

        for tt, a in sorted(abbo_agg.items()):
            q = int(a["q"])
            lordo = int(a["imp_fig"])  # cent
            iva_file = int(a["iva_fig"])  # cent
            ...
            if tipo_tass == "I":
                if iva_rate <= 0.0 and isi_rate <= 0.0:
                    issues.append(Issue(
                        severity="WARN",
                        check="FIS_RPM_ABBONAMENTI",
                        message="RPM: Abbonamenti (figurativi) presenti ma aliquote non disponibili: ISI/IVA calcolate=0 (usa TAB.1)",
                        context={"evento": f"{ek[2]} {ek[3]} {ek[1]}", "TipoTitolo": tt},
                    ))
                else:
                    calc = calc_intr_sp_from_gross(lordo, inc, float(iva_rate), float(isi_rate))
                    imp_isi = int(calc["imponibile_intr"]) if float(isi_rate) > 0.0 else 0
                    isi = int(calc["isi"])
                    imp_iva = int(calc["imponibile_intr"] + calc["imponibile_sp"])
                    iva = int(calc["iva_tot"])
                    if abs(iva - iva_file) > 1:
                        issues.append(Issue(
                            severity="WARN",
                            check="FIS_RPM_ABBONAMENTI",
                            message="IVAFigurativa diversa da IVA calcolata (calcolo-ISI) per Biglietti Abbonamento/AbbonamentiFissi",
                            context={"evento": f"{ek[2]} {ek[3]} {ek[1]}", "TipoTitolo": tt, "IVAFigurativa": iva_file, "IVA_calcolata": iva},
                        ))
            else:
                # Tassazione S (Spettacolo): ISI non prevista. Scorporo IVA sull'intero importo.
                if iva_rate > 0.0:
                    net = round_half_up_int(Decimal(lordo) / (Decimal('1') + Decimal(str(iva_rate))))
                    iva = int(lordo - net)
                    imp_iva = int(net)
                else:
                    imp_iva = int(lordo)
                    iva = int(iva_file) if iva_file else 0
            ...
            abbo_rows.append({
                ...
                "TitoliNetti": q,
                "TitoliAnnullati": 0,
                "ImponibileImposta": imp_isi,
                "ImpostaIntrattenimento": isi,
                "ImponibileIVA": imp_iva,
                "IVALorda": iva,
            })
            abbo_totals["TitoliNetti"] += q
            abbo_totals["ImponibileImposta"] += imp_isi
            abbo_totals["ImpostaIntrattenimento"] += isi
            abbo_totals["ImponibileIVA"] += imp_iva
            abbo_totals["IVALorda"] += iva

        totals_with_abbo = dict(totals)
        totals_with_abbo["TitoliNetti"] = int(totals.get("TitoliNetti", 0)) + int(abbo_totals["TitoliNetti"])
        totals_with_abbo["TitoliAnnullati"] = int(totals.get("TitoliAnnullati", 0)) + int(abbo_totals["TitoliAnnullati"])
        totals_with_abbo["ImponibileImposta"] = int(totals.get("ImponibileImposta", 0)) + int(abbo_totals["ImponibileImposta"])
        totals_with_abbo["ImpostaIntrattenimento"] = int(totals.get("ImpostaIntrattenimento", 0)) + int(abbo_totals["ImpostaIntrattenimento"])
        totals_with_abbo["ImponibileIVA"] = int(totals.get("ImponibileIVA", 0)) + int(abbo_totals["ImponibileIVA"])
        totals_with_abbo["IVALorda"] = int(totals.get("IVALorda", 0)) + int(abbo_totals["IVALorda"])

        fisc_ev_tables.append({**meta, "rows": rows, "totals": totals, "abbo_rows": abbo_rows, "totals_with_abbo": totals_with_abbo})



    # LOG-only: se non è presente RPM, ricostruiamo il riepilogo fiscale per evento/tipologia direttamente dai LOG.
    # In questo caso:
    # - IVA lorda viene presa dai campi LOG (IVACorrispettivo+IVAPrevendita)
    # - Imponibile intrattenimenti viene preso dal campo LOG ImponibileIntrattenimenti (per tassazione I)
    # - ISI viene calcolata applicando l'aliquota TAB.1 (se disponibile) all'imponibile intrattenimenti
    # - Imponibile IVA viene stimato come LORDO - IVA - ISI (coerente con "calcolo-ISI")
    # - La riga '* Eccedenza omaggi' è costruita usando i calcoli già prodotti nella sezione 8 (details['fiscale_omaggi']).

    if not rpm_recs:
        details["fiscale_mode"] = "LOG"

        # Somme eccedenza per evento (per riga '*') derivate dal dettaglio omaggi
        ecc_event = defaultdict(lambda: {"ImponibileImposta": 0, "ImpostaIntrattenimento": 0, "ImponibileIVA": 0, "IVALorda": 0})
        for rr in details.get("fiscale_omaggi", []) or []:
            ek = (rr.get("CF") or "", rr.get("CodiceLocale") or "", rr.get("DataEvento") or "", rr.get("OraEvento") or "")
            ecc = int(rr.get("Eccedenza") or 0)
            if ecc <= 0:
                continue
            imp_intr_unit = int(rr.get("ImponibileIntr_unit") or 0)
            imp_sp_unit = int(rr.get("ImponibileSp_unit") or 0)
            isi_unit = int(rr.get("ISI_unit") or 0)
            ecc_event[ek]["ImponibileImposta"] += int(rr.get("ImponibileIntr_addition") or 0)
            ecc_event[ek]["ImpostaIntrattenimento"] += ecc * isi_unit
            ecc_event[ek]["ImponibileIVA"] += ecc * (imp_intr_unit + imp_sp_unit)
            ecc_event[ek]["IVALorda"] += int(rr.get("Expected_IVAEccedenteOmaggi") or 0)
        ...
        fisc_ev_tables = []
        for ek in sorted(by_event.keys(), key=lambda x: (x[2], x[3], x[1], x[0])):
            rows_log = by_event[ek]
            cf, loc, data, ora = ek

            denom = Counter([str(r.get("DenominazioneLocale") or "").strip() for r in rows_log]).most_common(1)[0][0] if rows_log else ""
            tit_ev = Counter([str(r.get("TitoloEvento") or "").strip() for r in rows_log]).most_common(1)[0][0] if rows_log else ""
            inc = max([int(r.get("Incidenza") or 0) for r in rows_log] + [0])
            tass = Counter([str(r.get("TipoTassazione") or "").strip().upper() for r in rows_log]).most_common(1)[0][0] if rows_log else ""

            codes = [int(r.get("TipoGenere") or 0) for r in rows_log if int(r.get("TipoGenere") or 0) != 0]
            code = 0
            if codes:
                best = None
                for c, n in Counter(codes).most_common():
                    iva_r, isi_r = rates_for_code(c)
                    key = (1 if (tass == 'I' and isi_r > 0.0) else 0, n, c)
                    if best is None or key > best[0]:
                        best = (key, c)
                code = best[1] if best else Counter(codes).most_common(1)[0][0]

            iva_rate, isi_rate = rates_for_code(code)

            agg = defaultdict(lambda: {"q_acc": 0, "q_ann": 0, "corr": 0, "prev": 0, "iva_corr": 0, "iva_prev": 0, "imp_intr": 0})
            for r in rows_log:
                tipo = (r.get("TipoTitolo") or "").strip()
                is_ann = is_log_annullamento(r)
                sign = -1 if is_ann else 1
                if not is_ann:
                    agg[tipo]["q_acc"] += 1
                else:
                    agg[tipo]["q_ann"] += 1
                agg[tipo]["corr"] += sign * int(r.get("CorrispettivoLordo") or 0)
                agg[tipo]["prev"] += sign * int(r.get("Prevendita") or 0)
                agg[tipo]["iva_corr"] += sign * int(r.get("IVACorrispettivo") or 0)
                agg[tipo]["iva_prev"] += sign * int(r.get("IVAPrevendita") or 0)
                agg[tipo]["imp_intr"] += sign * int(r.get("ImponibileIntrattenimenti") or 0)

            rows = []
            totals = {"TitoliNetti": 0, "TitoliAnnullati": 0, "ImponibileImposta": 0, "ImpostaIntrattenimento": 0, "ImponibileIVA": 0, "IVALorda": 0}

            for tipo in sorted(agg.keys()):
                a = agg[tipo]
                q_net = int(a["q_acc"] - a["q_ann"])
                q_ann = int(a["q_ann"])
                corr = int(a["corr"])
                prev = int(a["prev"])
                iva_tot = int(a["iva_corr"] + a["iva_prev"])
                lordo = corr + prev
                ...
                if tass == 'I':
                    imp_isi = int(a["imp_intr"])
                    isi = round_half_up_int(Decimal(imp_isi) * Decimal(str(isi_rate))) if isi_rate > 0 else 0
                    imp_iva = int(lordo - iva_tot - isi)
                else:
                    imp_iva = int(lordo - iva_tot)

                rows.append({
                    "RowType": "log",
                    "TipoTitolo": tipo,
                    "TitoliNetti": q_net,
                    "TitoliAnnullati": q_ann,
                    "ImponibileImposta": imp_isi,
                    "ImpostaIntrattenimento": isi,
                    "ImponibileIVA": imp_iva,
                    "IVALorda": iva,
                ...
                totals["TitoliNetti"] += q_net
                totals["TitoliAnnullati"] += q_ann
                totals["ImponibileImposta"] += imp_isi
                totals["ImpostaIntrattenimento"] += isi
                totals["ImponibileIVA"] += imp_iva
                totals["IVALorda"] += iva

            ecc = ecc_event.get(ek)
            if ecc and any(int(ecc.get(k, 0) or 0) != 0 for k in ("ImponibileImposta", "ImpostaIntrattenimento", "ImponibileIVA", "IVALorda")):
                rows.append({
                    "RowType": "eccedenza",
                    "TipoTitolo": "* Eccedenza omaggi",
                    "TitoliNetti": 0,
                    "TitoliAnnullati": 0,
                    "ImponibileImposta": int(ecc.get("ImponibileImposta") or 0),
                    "ImpostaIntrattenimento": int(ecc.get("ImpostaIntrattenimento") or 0),
                    "ImponibileIVA": int(ecc.get("ImponibileIVA") or 0),
                    "IVALorda": int(ecc.get("IVALorda") or 0),
                })
                totals["ImponibileImposta"] += int(ecc.get("ImponibileImposta") or 0)
                totals["ImpostaIntrattenimento"] += int(ecc.get("ImpostaIntrattenimento") or 0)
                totals["ImponibileIVA"] += int(ecc.get("ImponibileIVA") or 0)
                totals["IVALorda"] += int(ecc.get("IVALorda") or 0)

            meta = {
                "CF": cf,
                "CodiceLocale": loc,
                "DenominazioneLocale": denom,
                "DataEvento": data,
                "OraEvento": ora,
                "TitoloEvento": tit_ev,
                "Incidenza": inc if inc else None,
                "TipoTassazione": tass,
            }
            fisc_ev_tables.append({**meta, "rows": rows, "totals": totals, "abbo_rows": [], "totals_with_abbo": totals})

        details["fiscale_evento_titoli"] = fisc_ev_tables
        metrics["FIS_event_tables"] = len(fisc_ev_tables)

        gt = {"TitoliNetti": 0, "TitoliAnnullati": 0, "ImponibileImposta": 0, "ImpostaIntrattenimento": 0, "ImponibileIVA": 0, "IVALorda": 0}
        for e in fisc_ev_tables:
            t = e.get("totals_with_abbo") or e.get("totals") or {}
            for k in gt.keys():
                gt[k] += int(t.get(k, 0) or 0)
        details["fiscale_evento_titoli_grand_totals"] = gt

    details["fiscale_evento_titoli"] = fisc_ev_tables
    metrics["FIS_event_tables"] = len(fisc_ev_tables)

    # Totale generale (tutti eventi): include abbonamenti + eccedenza omaggi
    gt = {"TitoliNetti": 0, "TitoliAnnullati": 0, "ImponibileImposta": 0, "ImpostaIntrattenimento": 0, "ImponibileIVA": 0, "IVALorda": 0}
    for e in fisc_ev_tables:
        t = e.get("totals_with_abbo") or e.get("totals") or {}
        gt["TitoliNetti"] += int(t.get("TitoliNetti", 0) or 0)
        gt["TitoliAnnullati"] += int(t.get("TitoliAnnullati", 0) or 0)
        gt["ImponibileImposta"] += int(t.get("ImponibileImposta", 0) or 0)
        gt["ImpostaIntrattenimento"] += int(t.get("ImpostaIntrattenimento", 0) or 0)
        gt["ImponibileIVA"] += int(t.get("ImponibileIVA", 0) or 0)
        gt["IVALorda"] += int(t.get("IVALorda", 0) or 0)
    details["fiscale_evento_titoli_grand_totals"] = gt
...
def build_html_report(summary: Dict[str, Any], issues: List[Issue], metrics: Dict[str, Any], details: Dict[str, Any]) -> str:
    ...
    if fisc_rows:
        ...
        for r in fisc_rows_sorted:
            rows_html.append(
                ...
                f"<td class='num'>{cents_to_eur_str(int(r.get('IVASp_unit') or 0))}</td>"
                f"<td class='num'>{cents_to_eur_str(int(r.get('IVAUnit_ecc') or 0))}</td>"
                f"<td>{'Sì' if r.get('Soglia_applicata_intr') else 'No'}</td>"
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class='num'>IVA%</th>
                <th class='num'>ISI%</th>
                <th class='num'>Imp. unit intr</th>
                <th class='num'>IVA unit intr</th>
                <th class='num'>IVA unit sp</th>
                <th class='num'>IVA unit ecc</th>
                <th>Soglia intr</th>
                <th>Soglia sp</th>
    ...
    # Riepilogo fiscale per evento/tipologia
    fisc_mode = (details.get('fiscale_mode') or ('RPM' if int(metrics.get('RPM_records',0) or 0) > 0 else 'LOG')).upper()
    ev_tables = details.get("fiscale_evento_titoli", [])
    grand = details.get("fiscale_evento_titoli_grand_totals", {}) or {}
    ev_tables_html = ""
    if ev_tables:
        ...
        for e in ev_tables_sorted:
            ...
            abbo_rows = e.get("abbo_rows", []) or []
            totals_with_abbo = e.get("totals_with_abbo", {}) or totals
            row_html: List[str] = []
            for r in rows:
                row_cls = ""
                if str(r.get("RowType", "")) == "eccedenza" or str(r.get("TipoTitolo", "")).strip().startswith("*"):
                    row_cls = "ecc"
            ...
            # --- Abbonamenti: dopo il Totale evento
            if abbo_rows:
                row_html.append("<tr class='sep'><td colspan='7'><b>Biglietti Abbonamento (figurativo)</b></td></tr>")
                for r in abbo_rows:
                    row_html.append(
                        "<tr class='abbo'>"
                        + f"<td><code>{html.escape(str(r.get('TipoTitolo','')))}</code></td>"
                ...
                row_html.append(
                    "<tr class='tot2'>"
                    "<td><b>Totale evento (incl. abbonamenti)</b></td>"
                    f"<td class='num'><b>{html.escape(str(totals_with_abbo.get('TitoliNetti',0)))}</b></td>"
                    f"<td class='num'><b>{html.escape(str(totals_with_abbo.get('TitoliAnnullati',0)))}</b></td>"
                    f"<td class='num'><b>{cents_to_eur_str(int(totals_with_abbo.get('ImponibileImposta',0) or 0))}</b></td>"
                    f"<td class='num'><b>{cents_to_eur_str(int(totals_with_abbo.get('ImpostaIntrattenimento',0) or 0))}</b></td>"
                    f"<td class='num'><b>{cents_to_eur_str(int(totals_with_abbo.get('ImponibileIVA',0) or 0))}</b></td>"
                    f"<td class='num'><b>{cents_to_eur_str(int(totals_with_abbo.get('IVALorda',0) or 0))}</b></td>"
                    "</tr>"
                )
        ...
        if grand:
            ...
            <div class=\"table-wrap\" style=\"margin-top:10px\">
              <table>
                <thead>
                  <tr>
                    <th><b>Totale generale (tutti eventi, incl. abbonamenti + eccedenza omaggi)</b></th>
                    <th class='num'>Titoli netti</th>
                    <th class='num'>Annullati</th>
                    <th class='num'>Imponibile imposta</th>
                    <th class='num'>Imposta intratt.</th>
                ...
                <tbody>
                  <tr class='totG'>
                    <td><b>Somma</b></td>
                    <td class='num'><b>{html.escape(str(grand.get('TitoliNetti',0)))}</b></td>
                    <td class='num'><b>{html.escape(str(grand.get('TitoliAnnullati',0)))}</b></td>
                    <td class='num'><b>{cents_to_eur_str(int(grand.get('ImponibileImposta',0) or 0))}</b></td>
                    <td class='num'><b>{cents_to_eur_str(int(grand.get('ImpostaIntrattenimento',0) or 0))}</b></td>
                    <td class='num'><b>{cents_to_eur_str(int(grand.get('ImponibileIVA',0) or 0))}</b></td>
                    <td class='num'><b>{cents_to_eur_str(int(grand.get('IVALorda',0) or 0))}</b></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        ev_tables_html = f"""
        <details open>
          <summary><b>Riepilogo fiscale per evento e tipologia titolo ({'LOG' if fisc_mode == 'LOG' else 'RPM'}, netto annulli + *eccedenza omaggi*)</b> (eventi: {len(ev_tables_sorted)})</summary>
          <div class="hint">
            { "Valori calcolati dallo script sui LOG (Corrispettivo+Prevendita) al netto degli annulli. L'IVA è presa dai campi LOG, l'ISI (se tassazione I) è ricavata applicando TAB.1 (CSV/PDF) all'ImponibileIntrattenimenti del LOG." if fisc_mode=='LOG' else "Valori calcolati dallo script sugli aggregati RPM (Corrispettivo+Prevendita), usando Incidenza da RPM e aliquote da TAB.1." }
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--log-all", action="store_true", help="Usa TUTTI i file LOG trovati senza selezione per giorno/versione (default: per ogni giorno prende il contatore più alto).")
    ap.add_argument("--merged-log-out", default=None, help="(Opzionale) Percorso file per scrivere un LOG unificato (XML) con le transazioni dei LOG selezionati.")
    ...
    # Fallback: se non è disponibile il CSV, prova a leggere TAB.1 da PDF (se presente in cartella)
    if aliquote is None:
        tab1_pdf = _find_tab1_pdf(directory)
        if tab1_pdf and os.path.exists(tab1_pdf):
            try:
                aliquote = parse_aliquote_tab1_pdf(tab1_pdf)
                print(f"[INFO] Aliquote TAB.1 caricate da PDF: {os.path.basename(tab1_pdf)}")
            except Exception as e:
                print(f"[WARN] Impossibile caricare TAB.1 da PDF ({tab1_pdf}): {e}")
    log_paths_found = list(files["LOG"])
    log_paths_selected: List[str] = []
    log_paths_ignored: List[str] = []
    log_select_info: Dict[str, Any] = {}

    if args.log_all:
        log_paths_selected = log_paths_found
    else:
        log_paths_selected, log_paths_ignored, log_select_info = select_latest_log_per_day(log_paths_found)

    log_recs: List[Dict[str, Any]] = []
    for p in log_paths_selected:
        log_recs.extend(parse_log(p))

    if args.merged_log_out:
        try:
            write_merged_log_xml(log_paths_selected, args.merged_log_out)
        except Exception as e:
            print(f"[WARN] Impossibile scrivere merged LOG: {e}")
    ...
    # Metriche: LOG selezionati
    metrics["LOG_files_found"] = len(log_paths_found)
    metrics["LOG_files_selected"] = len(log_paths_selected)
    metrics["LOG_files_selected_list"] = ", ".join([os.path.basename(p) for p in log_paths_selected])
    metrics["LOG_files_ignored"] = ", ".join([os.path.basename(p) for p in log_paths_ignored]) if log_paths_ignored else ""
    if log_select_info.get("non_conformi"):
        metrics["LOG_files_non_conformi_nome"] = ", ".join(log_select_info.get("non_conformi", []))
