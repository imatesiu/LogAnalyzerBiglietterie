...
- Soglia “modico valore” (default 25,82 €) applicata sull’**imponibile intrattenimenti unitario**:
    se imponibile_unitario <= soglia => IVAEccedenteOmaggi = 0
"""
...
def parse_lta(path: str) -> List[Dict[str, Any]]:
    ...
    root = tree.getroot()
    out: List[Dict[str, Any]] = []
    for ta in root.findall(".//TitoloAccesso"):
        ...
        out.append({
            "_file": os.path.basename(path),
            "key": key,
            "StatoTitolo": (ta.get("StatoTitolo") or "").strip(),
            "Annullamento": (ta.get("Annullamento") or "").strip(),
            "CodiceLocale": (ta.get("CodiceLocale") or "").strip(),
            "DataEvento": (ta.get("DataEvento") or "").strip(),
            "OraEvento": (ta.get("OraEvento") or "").strip(),
...
def parse_rca(path: str) -> List[Dict[str, Any]]:
    ...
    for ev in root.findall(".//Evento"):
        evento_info = {
            "_file": os.path.basename(path),
            "CFTitolare": _find_text(ev, "CFTitolare"),
            "CodiceLocale": _find_text(ev, "CodiceLocale"),
            "DataEvento": _find_text(ev, "DataEvento"),
            "OraEvento": _find_text(ev, "OraEvento"),
            "CodiceOrdine": _find_text(ev, "CodiceOrdine"),
        }
        for rie in ev.findall(".//Riepilogo"):
            out.append({
                **evento_info,
                "TipoTitolo": _find_text(rie, "TipoTitolo"),
                "StatoTitolo": _find_text(rie, "StatoTitolo"),
                "Quantita": to_int(_find_text(rie, "Quantita"), 0),
            })
    return out
...
CHECK_DEFS: Dict[str, str] = {
    ...
    "LOG_vs_LTA": "LOG↔LTA: titoli emessi/annullati presenti in lista titoli",
    "LTA_vs_RCA": "LTA↔RCA: coerenza conteggi per evento/ordine/stato",
    "RCA_internal": "RCA: coerenza somme interne (somma stati = totale)",
...
def run_checks(
    ...
    # -------------------------
    # 3) LOG vs LTA presence (emissioni e annullamenti)
    # -------------------------
    for r in log_recs:
        k: TicketKey = r["key"]
        if k not in lta_by_key:
            issues.append(Issue(
                severity="WARN",
                check="LOG_vs_LTA",
                message="Titolo presente nel LOG ma assente in LTA",
                context={"key": k.short(), "file": r.get("_file"), "annullamento": r.get("Annullamento")},
            ))

    # -------------------------
    # 4) LTA vs RCA (conteggi per evento/ordine/stato)
    # -------------------------
    # LTA counts
    lta_counts: Dict[Tuple[str, str, str, str], Counter] = defaultdict(Counter)
    for r in lta_recs:
        k = (r.get("CodiceLocale") or "", r.get("DataEvento") or "", r.get("OraEvento") or "", r.get("CodiceOrdine") or "")
        stato = r.get("StatoTitolo") or ""
        lta_counts[k][stato] += 1

    # RCA counts
    rca_counts: Dict[Tuple[str, str, str, str], Counter] = defaultdict(Counter)
    for r in rca_recs:
        k = (r.get("CodiceLocale") or "", r.get("DataEvento") or "", r.get("OraEvento") or "", r.get("CodiceOrdine") or "")
        stato = r.get("StatoTitolo") or ""
        rca_counts[k][stato] += int(r.get("Quantita") or 0)

    # compare only keys that exist in either
    all_keys = set(lta_counts.keys()) | set(rca_counts.keys())
    for k in sorted(all_keys):
        st_lta = lta_counts.get(k, Counter())
        st_rca = rca_counts.get(k, Counter())
        # compare per stato presente
        st_all = set(st_lta.keys()) | set(st_rca.keys())
        for stato in st_all:
            if st_lta.get(stato, 0) != st_rca.get(stato, 0):
                issues.append(Issue(
                    severity="WARN",
                    check="LTA_vs_RCA",
                    message="Differenza quantità per stato tra LTA e RCA",
                    context={"key": f"{k}", "stato": stato, "lta": st_lta.get(stato, 0), "rca": st_rca.get(stato, 0)},
                ))

    # internal RCA: sum of stati per (evento/ordine) = totale? (se presente)
    # Nello schema RCA non c'è un totale esplicito per evento/ordine; facciamo un controllo "di coerenza"
    # su stati noti: EMESSO, CONTROLLATO, ANNULLATO, ecc.
    # Qui mettiamo un INFO se ci sono stati "strani" non riconosciuti.
    known_states = {"EMESSO", "CONTROLLATO", "ANNULLATO", "SOSPESO", "SCADUTO", "RIMBORSATO"}
    for k, counter in rca_counts.items():
        unknown = [s for s in counter.keys() if s and s.upper() not in known_states]
        if unknown:
            issues.append(Issue(
                severity="INFO",
                check="RCA_internal",
                message="RCA contiene stati titolo non standard (verificare se previsto dal gestionale)",
                context={"key": f"{k}", "stati": ", ".join(sorted(set(unknown)))},
            ))
    ...
    # -------------------------
    # 6) LTA vs RPM (quantità)
    # -------------------------
    # LTA: emessi e annullati
    lta_emessi = 0
    lta_annullati = 0
    for r in lta_recs:
        # Heuristica: StatoTitolo == ANNULLATO oppure Annullamento == S
        stato = (r.get("StatoTitolo") or "").upper()
        ann = (r.get("Annullamento") or "").upper()
        if stato == "ANNULLATO" or ann == "S":
            lta_annullati += 1
        else:
            lta_emessi += 1
    ...
    rpm_tot_accesso = 0
    rpm_tot_annullati = 0
    for (k_kind, *_rest), rr in rpm_by_key.items():
        if k_kind == "TitoliAccesso":
            rpm_tot_accesso += int(rr.get("Quantita") or 0)
        elif k_kind == "TitoliAnnullati":
            rpm_tot_annullati += int(rr.get("Quantita") or 0)

    metrics["LTA_emessi"] = lta_emessi
    metrics["LTA_annullati"] = lta_annullati
    metrics["RPM_tot_accesso"] = rpm_tot_accesso
    metrics["RPM_tot_annullati"] = rpm_tot_annullati

    if lta_emessi != rpm_tot_accesso:
        issues.append(Issue(
            severity="WARN",
            check="LTA_vs_RPM",
            message="Totale titoli emessi (LTA) diverso dal totale TitoliAccesso (RPM)",
            context={"lta_emessi": lta_emessi, "rpm_accesso": rpm_tot_accesso},
        ))
    if lta_annullati != rpm_tot_annullati:
        issues.append(Issue(
            severity="WARN",
            check="LTA_vs_RPM",
            message="Totale titoli annullati (LTA) diverso dal totale TitoliAnnullati (RPM)",
            context={"lta_annullati": lta_annullati, "rpm_annullati": rpm_tot_annullati},
        ))
    ...
    for ok in sorted(all_order_keys):
        ...
        # regola soglia
        if imponibile_intr_unit <= soglia_omaggi_cents:
            expected_iva_ecc = 0
            soglia_flag = True
        else:
        ...
        if rpm_iva_ecc is not None and int(rpm_iva_ecc) != int(expected_iva_ecc):
            ...
            issues.append(Issue(
                severity=sev,
                check="FIS_RPM_OMAGGI",
                message="RPM: IVAEccedenteOmaggi non coerente con calcolo eccedenza omaggi (prezzo max, no prevendita)",
                context={
                    "evento": f"{data} {ora} {loc} Ord={ordine}",
                    "capienza": capienza,
                    "omaggi_net": om_log,
    ...
    if fisc_rows:
        ...
        <details open>
          ...
          <div class="hint">
            ...
            Soglia imponibile unitario: {params.get('soglia_omaggi_eur',25.82):.2f} €.
...
<div class="wrap">
  ...
  <div class="hint" style="margin-top:16px;">
    Parametri: omaggi_pct_capienza={params.get('omaggi_pct_capienza',5)}, soglia_omaggi={params.get('soglia_omaggi_eur',25.82)} €.
  </div>
</div>
</body>
</html>
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--soglia-omaggi", type=float, default=25.82, help="Soglia imponibile unitario (EUR) per IVAEccedenteOmaggi=0 (default 25.82)")
